...
func add_group():
	get_node("Boss_5_KinematicBody").add_to_group("enemies")

func check_visible():
	if get_node("Boss_5_Visibility").is_on_screen():
		is_visible = true
	if not get_node("Boss_5_Visibility").is_on_screen():
		is_visible = false

	if is_visible == true:
		get_node("Boss_5_KinematicBody/Boss_5_Animation").play("move")
	if is_visible == false:
		get_node("Boss_5_KinematicBody/Boss_5_Animation").stop()

func _on_Boss_5_Hit_area_entered(area):
	if area.is_in_group("player_attack") and can_hurt == true:
		can_hurt = false
		get_node("Boss_Hit_SFX").play()
		get_node("Boss_5_KinematicBody/Boss_5_Sprite").set_modulate(Color(0,0,0))
		...
		get_node("Boss_5_KinematicBody/Boss_5_Sprite").set_modulate(Color(1,1,1))
...
func check_player_win():
	if life <= 0:
		can_play_death = true
		play_death()
		get_node("Boss_5_KinematicBody/Boss_5_Col").disabled = true
		get_node("Boss_5_KinematicBody/Boss_5_Hit/Boss_5_Col_Hit").disabled = true
		get_node("Boss_5_KinematicBody/Boss_5_Sprite").visible = false
		var waiting_timer = Timer.new()
